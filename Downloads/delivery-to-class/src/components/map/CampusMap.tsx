...
export const CampusMapContent: React.FC<CampusMapProps> = ({
  ...
  useEffect(() => {
    ...
    // Cleanup existing instance and DOM leaflet ID
    if (mapInstance) {
      mapInstance.remove();
    }
    ...
    const map = LState.map(mapContainer, {
  ...
  }, [LState, shops, userLocation, courierLat, courierLng, courierName, destinationLat, destinationLng, onSelectShop]);
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-800 shadow-md">
      ...
      {/* Leaflet map div */}
      <div ref={containerRef} style={{ height, width: '100%' }} className="bg-slate-100 dark:bg-slate-900" />
    </div>
  );
};
